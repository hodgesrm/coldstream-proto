...
"""Grand Coulee invoice analyzer

Grand Coulee is a fake company used for testing
"""
...
class GrandCouleeProcessor:
    """Grand Coulee Invoice Processor"""
    ...
    @staticmethod
    def conforms(model):
        """Returns True if we can find www.gchosting.com in the first page."""

        engine = tq.QueryEngine(model)
        count = 0
        lines = engine.tabular_query().matches_type(tm.Line).page(
            1).matches_regex(
            r'www\.gchosting\.net').generate()
        for line in lines:
            logger.debug(line)
            count += 1
        logger.debug("count of www.gchosting.net: {0}".format(count))
        return count >= 1
    ...
    def get_content(self):
        ...
        identifier_region = tm.Region(left=2052, top=212, right=2338,
                                      bottom=243)
        eff_date_region = tm.Region(left=2054, top=271, right=2271, bottom=300)
        ...
        invoice.identifier = re.sub(r'^\s*Invoice\s*', '', identifier)
        ...
        total_amount_regex = r'^\s*Total:'
        subtotal_amount_regex = r'^\s*Subtotal:'
        ...
        # Grand Coulee currently does not show tax.
        invoice.tax = '0.0'
        ...
        # bottom of the line that contains the word 'Service'.  We add
        # extra pixels to ensure we're below it.
        first_page = 1
        first_page_top = 0
        extra_pixels = 5
        ...
        # Similarly get the bottom which must be above the subtotal line.
        last_page = 0
        last_page_bottom = 0
        ...
        # with the word 'Bare Metal'.
        left_margin_regex = r'^Bare Metal'
        left_margin = 99999
        for line in self._engine.tabular_line_query().matches_regex(
        ...
        # phrase 'Service'.  We query from the second page to get
        # an accurate view of the header, which is lower on page 1.
        top_margin_regex = '^Service'
        top_margin = 99999
        for line in self._engine.tabular_line_query().page(2).matches_regex(
                top_margin_regex).generate():
            top_margin = min(top_margin, line.region.bottom)

        # The bottom edge is the top value of any row with the hosting
        # company name at the top of the page.
        bottom_margin_regex = r'^(Grand Coolee Hosting|Grand Coulee Hosting)'
        bottom_margin = 0
        for line in self._engine.tabular_line_query().page(1).matches_regex(
                bottom_margin_regex).generate():
            bottom_margin = max(bottom_margin, line.region.top)
        bottom_margin += 10
        ...
        # Iterate over the invoice line items.  The left edge is a constant.
        invoice_item_left_edge = 500
        ...
        for line in self._engine.tabular_line_query().intersects_vertical_edge(
            ...
            if len(item_list) > 1:
                ...
                if candidate_start_date is not None:
                    ...
                    # for the resource id.
                    logger.debug(
                        "PARSING--starting date: {0}".format(
                ...
                item_units = data_utils.extract_integer(item_list[-2].text)
                item_unit_amount = data_utils.extract_currency(item_list[-3].text)
