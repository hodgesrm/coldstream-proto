...
if [ "$OPT" = "clean" ]; then
  ...
  shift
  args="$@ generate -t $CODEGEN_HOME/modules/swagger-codegen/src/main/resources/JavaJaxRS -i $SWAGGER_HOME/goldfin-service-admin-api.yaml -l jaxrs -o $API_HOME -DhideGenerationTimestamp=true,serverPort=8080 --model-package=io.goldfin.admin.service.api.model --api-package=io.goldfin.admin.service.api.service --invoker-package=io.goldfin.admin.service.api.invoker"
else
