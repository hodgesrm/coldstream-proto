...
public class VendorDataService implements TransactionalService<Vendor> {
	...
	public Vendor get(String id) {
		TabularResultSet result = new SqlSelect().from("vendors").project(COLUMN_NAMES).whereId(UUID.fromString(id))
				.run(session);
	...
	public Vendor getByIdentifier(String identifier) {
		TabularResultSet result = new SqlSelect().from("vendors").project(COLUMN_NAMES).where("identifier = ?", identifier)
				.run(session);
		if (result.rowCount() == 0) {
			return null;
		} else {
	...
	public List<Vendor> getAll() {
		TabularResultSet result = new SqlSelect().from("vendors").project(COLUMN_NAMES).run(session);
