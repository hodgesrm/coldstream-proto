...
public class SessionDataService implements TransactionalService<SessionData> {
	...
	public SessionData get(String id) {
		TabularResultSet result = new SqlSelect().table("sessions").get(COLUMN_NAMES).id(UUID.fromString(id))
				.run(session);
	...
	public List<SessionData> getAll() {
		TabularResultSet result = new SqlSelect().table("sessions").get(COLUMN_NAMES).run(session);
