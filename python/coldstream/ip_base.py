...
class Repo:
    ...
    def create_repo(repo_cfg):
        ...
        if os.path.exists(repo_cfg):
            raise BaseError("Repo already exists: {0}".format(root.cfg))
    ...
    def delete_repo(repo_cfg, force=False):
        ...
        # Read the config and delete the metadata and docs directories + the
        # repo config itself. 
        repo = Repo(repo_cfg)
        document_dir = repo.get_cfg_value("documents", "dir")
        metadata_dir = repo.get_cfg_value("metadata", "dir")
        if os.path.exists(document_dir):
            shutil.rmtree(document_dir, force)
        if os.path.exists(metadata_dir):
            shutil.rmtree(metadata_dir, force)
        os.remove(repo_cfg)
    ...
    def validate(self):
        ...
        ok = True
        messages = []
        document_dir = self.get_cfg_value("documents", "dir")
        if document_dir == None:
            messages.append(self._validation_message("documents", "dir", "Value is missing"))
            ok = False
        elif not os.path.exists(document_dir):
            messages.append(self._validation_message("documents", "dir", "Value is missing"))
            ok = False
        else: 
            messages.append(self._validation_message("documents", "dir", "OK"))

        metadata_dir = self.get_cfg_value("metadata", "dir")
        if metadata_dir == None:
            messages.append(self._validation_message("metadata", "dir", "Value is missing"))
            ok = False
        elif not os.path.exists(metadata_dir):
            messages.append(self._validation_message("metadata", "dir", "Value is missing"))
            ok = False
        else: 
            messages.append(self._validation_message("metadata", "dir", "OK"))
